      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="page-header">
          <div>
            ...
            <p class="page-subtitle">Administración de tipos de vehículos</p>
          </div>
          <Link :href="route('admin.tipos-vehiculo.create')">
        ...
        <Card :padding="false">
          <div class="overflow-x-auto">
            <table class="table">
              ...
              <tbody class="table-body">
                <tr v-for="tipo in (tipos.data || tipos)" :key="tipo.id" class="table-row">
                  <td class="table-cell">{{ tipo.id }}</td>
                  <td class="table-cell font-medium">{{ tipo.nombre }}</td>
                  <td class="table-cell-secondary">{{ tipo.descripcion }}</td>
                  <td class="table-cell">
                    <div class="flex items-center justify-end gap-2">
                      <Link :href="route('admin.tipos-vehiculo.edit', tipo.id)">
          ...
          <div v-if="tipos.data" class="p-4 border-t border-theme">
            <Pagination :data="tipos" />
          </div>
        </Card>
      </div>
    </div>
...
defineProps<{ tipos: any }>();

const deleteTipo = (id: number) => {
  if (confirm('¿Eliminar este tipo de vehículo?')) {
    router.delete(route('admin.tipos-vehiculo.destroy', id));
  }
};
</script>
