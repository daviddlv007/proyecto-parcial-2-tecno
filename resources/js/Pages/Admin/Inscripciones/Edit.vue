    ...
    <div class="page-container py-6">
      <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="page-header mb-6">
          <h1 class="page-title">Editar Inscripción</h1>
          <p class="page-subtitle">Actualizar información de la inscripción</p>
        </div>
        ...
        <Card>
          <form @submit.prevent="submit" class="p-6 space-y-6">
            ...
            <div class="space-y-2">
              ...
              />
              <div v-if="inscripcion.edicion" class="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                <p class="text-xs text-blue-700 dark:text-blue-300">
                  <strong>Edición actual:</strong> {{ inscripcion.edicion.curso?.nombre }} - 
                  {{ formatDate(inscripcion.edicion.fecha_inicio) }} a {{ formatDate(inscripcion.edicion.fecha_fin) }}
                </p>
              </div>
            </div>
            ...
            <div class="form-actions">
              ...
              <Button variant="primary" type="submit" :disabled="form.processing">
                {{ form.processing ? 'Actualizando...' : 'Actualizar Inscripción' }}
              </Button>
            </div>
          </form>
        </Card>
...
const form = useForm({
  alumno_id: props.inscripcion?.alumno_id || null,
  curso_edicion_id: props.inscripcion?.curso_edicion_id || null,
  plan_pago_id: props.inscripcion?.plan_pago_id || null,
  estado_inscripcion: props.inscripcion?.estado_inscripcion || 'pendiente'
});
...
const submit = () => {
  form.put(route('admin.inscripciones.update', props.inscripcion.id));
};
</script>
